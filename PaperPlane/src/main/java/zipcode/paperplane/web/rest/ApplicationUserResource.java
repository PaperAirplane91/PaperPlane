...
import zipcode.paperplane.repository.ApplicationUserRepository;
import zipcode.paperplane.web.rest.errors.BadRequestAlertException;

/**
 * REST controller for managing {@link zipcode.paperplane.domain.ApplicationUser}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<ApplicationUser> partialUpdateApplicationUser(
        ...
        Optional<ApplicationUser> result = applicationUserRepository
            ...
            .map(existingApplicationUser -> {
                if (applicationUser.getApplicationUserId() != null) {
                    existingApplicationUser.setApplicationUserId(applicationUser.getApplicationUserId());
                }
                if (applicationUser.getAccessControl() != null) {
                    existingApplicationUser.setAccessControl(applicationUser.getAccessControl());
                }

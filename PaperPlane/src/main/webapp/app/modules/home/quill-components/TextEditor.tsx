...
const handleDelete = async () => {
  if (selectedDocumentId !== null) {
    try {
      const response = await fetch(`https://pp.zipcode.rocks/api/documents/${selectedDocumentId}`, {
        method: 'DELETE',
      });
...
const handleDocumentSelect = async (id: number) => {
    try {
      ...
      const response = await fetch(`https://pp.zipcode.rocks/api/documents/${id}`);
      if (response.ok) {
...
const handleFileUpload = async (file) => {
  try {
    ...
    const response = await fetch('https://pp.zipcode.rocks/api/documents/upload', {
      method: 'POST',
      body: formData,
    });
